...
class Multi2SingleEnv(Wrapper):
    ...
    def __init__(self, env, env_name, agent, agent_idx, shaping_params, scheduler, total_step, norm=True,
                 ...
                 mix_agent=False, mix_ratio=0.5, _agent=None, mode="abs", reward_str=None):
        ...
        if reward_str:
            ...
            reward_calc = local_scope["RewardCalculator"]()  # 实例化
            self.reward_function = reward_calc.compute        # 绑定成函数使用
    ...
    def step(self, action):
        ...
        # print(self.cnt, self.total_step)
        ...
        if self.norm and self.mode == "abs":
        ...
        elif "oppo" in self.mode:
            return ob, reward, done, info
        elif "llm" in self.mode:
            reward, details = self.reward_function(self.nowob, ob, action, winlabel, self.ob, self.abs_reward + self.oppo_reward, self.oppo_reward)
            self.nowob = ob
            info['details'] = details
            return ob, reward, done, info
        else:
             raise ValueError(f"Unknown mode: {self.mode}. Expected 'abs' or 'oppo'.")
